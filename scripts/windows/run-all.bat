...
if "%NODE_INSTANCE%"=="" (
    start "EdgeNode" cmd /k cd /d "%ROOT_DIR%" ^&^& java -cp "%CP%" components.node.EdgeNode
) else (
    start "EdgeNode" cmd /k cd /d "%ROOT_DIR%" ^&^& java -cp "%CP%" components.node.EdgeNode %NODE_INSTANCE%
)
