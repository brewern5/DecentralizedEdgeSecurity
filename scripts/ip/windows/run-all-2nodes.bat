...
start "EdgeNode node1" cmd /k cd /d "%ROOT_DIR%" ^&^& java %COMMON_JAVA_OPTS% -cp "%CP%" components.node.EdgeNode node1
start "EdgeNode node2" cmd /k cd /d "%ROOT_DIR%" ^&^& java %COMMON_JAVA_OPTS% -cp "%CP%" components.node.EdgeNode node2
