...
if "%2"=="" (
    set /p NODE_INSTANCE="Enter Node Instance ID (leave blank for default): "
) else (
    set "NODE_INSTANCE=%2"
)
...
if "%NODE_INSTANCE%"=="" (
    start "EdgeNode" cmd /k cd /d "%ROOT_DIR%" ^&^& java %COMMON_JAVA_OPTS% -cp "%CP%" components.node.EdgeNode
) else (
    start "EdgeNode" cmd /k cd /d "%ROOT_DIR%" ^&^& java %COMMON_JAVA_OPTS% -cp "%CP%" components.node.EdgeNode %NODE_INSTANCE%
)
