...
tmux new-session -d -s des "cd '${ROOT_DIR}' && java -cp '${CP}' components.coordinator.EdgeCoordinator"
if [[ -z "${SERVER_INSTANCE}" ]]; then
  tmux new-window -t des "cd '${ROOT_DIR}' && java -cp '${CP}' components.server.EdgeServer"
else
  tmux new-window -t des "cd '${ROOT_DIR}' && java -cp '${CP}' components.server.EdgeServer ${SERVER_INSTANCE}"
fi
if [[ -z "${NODE_INSTANCE}" ]]; then
  tmux new-window -t des "cd '${ROOT_DIR}' && java -cp '${CP}' components.node.EdgeNode"
else
  tmux new-window -t des "cd '${ROOT_DIR}' && java -cp '${CP}' components.node.EdgeNode ${NODE_INSTANCE}"
fi
